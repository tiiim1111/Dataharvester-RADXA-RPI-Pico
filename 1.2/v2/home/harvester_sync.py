...
ALLOWED_TOP = set(SYNC_FOLDERS) | {"settings"}
SKIP_NAMES  = {"System Volume Information", "$RECYCLE.BIN", "lost+found"}
...
def refresh_view():
    ...
    if not is_root():
        ...
        return
    subprocess.run(
        ["mount", "-o", f"remount,ro", str(MOUNTPOINT)],
        text=True,
...
def remount_sd_ro():
    """
    Best-effort: unmount and remount the SD card read-only to refresh the view.
    This assumes the device is SD_DEVICE and the mountpoint is MOUNTPOINT.
    """
    try:
        res = subprocess.run(
        ...
        if res.returncode != 0 and "not mounted" not in res.stderr.lower():
            log.warning(f"umount {MOUNTPOINT} returned rc={res.returncode}: {res.stderr.strip()}")
        else:
            log.info(f"Unmounted {MOUNTPOINT} (or it was already unmounted).")
    except Exception as e:
        log.warning(f"Exception during umount {MOUNTPOINT}: {e}")

    time.sleep(2)

    try:
        res = subprocess.run(
            ["mount", "-o", "ro", SD_DEVICE, str(MOUNTPOINT)],
            text=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            check=False,
        ...
        if res.returncode != 0:
            log.error(f"Failed to mount {SD_DEVICE} on {MOUNTPOINT} (ro): {res.stderr.strip()}")
        else:
            log.info(f"Mounted {SD_DEVICE} on {MOUNTPOINT} as read-only.")
    except Exception as e:
        log.error(f"Exception during mount {SD_DEVICE} -> {MOUNTPOINT}: {e}")
...
def main():
    ...
    while not _stop:
        ...
        log.info(
            "dest_root: %s | source_root: %s | folders=%s | interval_seconds=%s | checksum=%s",
