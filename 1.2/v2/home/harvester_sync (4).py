...
SD_DEVICE    = "/dev/mmcblk1p1"   # <- add this
...
def remount_sd_ro():
    ...
    # Try to unmount; ignore failure but log it
    try:
        res = subprocess.run(
    ...
    # Wait 2 seconds before attempting to mount again
    time.sleep(2)

    # Now try to mount read-only
    try:
        res = subprocess.run(
...
def main():
    ...
    while not _stop:
        ...
        for folder in SYNC_FOLDERS:
            ...
            # Optional: enable checksum-based change detection
            # if USE_CHECKSUM:
