...
APPLY_SCRIPT = Path("/home/radxa/scripts/harvester_msc_default_folders.py")

BASE_REQUIRED = ["cal_log", "datalog", "unclassified", "settings", "settings/logs", "alarms_data"]
    ...
    if res.returncode != 0:
        if not is_root() and res.returncode == 1 and "a password is required" in (res.stderr.lower() or ""):
            log.error(
                ...
                "Run once as root: sudo -E /usr/bin/env python3 /home/radxa/scripts/harvester_msc_default_folders.py\n"
                "Or run this watcher as a root systemd service (see unit file below), "
                "or grant passwordless sudo for this script:\n"
                "  echo 'radxa ALL=(root) NOPASSWD: /usr/bin/env python3 /home/radxa/scripts/harvester_msc_default_folders.py' | sudo tee /etc/sudoers.d/harvester\n"
            )
        else:
